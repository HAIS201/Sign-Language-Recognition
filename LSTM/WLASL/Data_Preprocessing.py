...
def z_rotate(X, angle_deg=10):
    ...
    if X.ndim == 3 and X.shape[1:] == (543, 3):
        T = X.shape[0]
        coords = X
        ...
        return rotated.reshape(T, -1).astype(np.float32)

    elif X.ndim == 2 and X.shape[1] == 1629:
        T = X.shape[0]
        coords = X.reshape(T, -1, 3)
        theta = np.deg2rad(np.random.uniform(-angle_deg, angle_deg))
        rot_matrix = np.array([
            [np.cos(theta), -np.sin(theta), 0],
            [np.sin(theta),  np.cos(theta), 0],
...
for action in actions:
    ...
    print(f"{action} 처리 완료 — 총 {len(sequence_folders)}개 시퀀스 사용됨\n")
    
sequences = [[frame.reshape(-1) for frame in seq] for seq in sequences]
...
y = to_categorical(labels, num_classes=50).astype(np.float32)
...
# 데이터 저장
save_path = save_path = "C:/Users/2reny/Desktop/LSTM3/PadData"
...
X_train_flat = X_train_pad.reshape(X_train_pad.shape[0], X_train_pad.shape[1], -1)
X_train_flat = X_train_flat.reshape(-1, X_train_flat.shape[-1])
...
X_test_flat = X_test_pad.reshape(X_test_pad.shape[0], X_test_pad.shape[1], -1)
X_test_flat = X_test_flat.reshape(-1, X_test_flat.shape[-1])
...
np.save(f"{save_path}/X_train_selected.npy", X_train_selected)
np.save(f"{save_path}/X_test_selected.npy", X_test_selected)