...
from utils import (
    plot_confusion_matrix, print_classification_report, seed_everything, select_topk_lightgbm, select_topk_mutual_info
)
...
def train():
    ...
    DATA_ROOT = r"D:\pycharm\transformer\sign_language_transformer\data"
    TRAIN_DIR = f"{DATA_ROOT}\\MP_Data"
    TEST_DIR = f"{DATA_ROOT}\\MP_Data_Test"
    TRAIN_XLSX = f"{DATA_ROOT}\\KETI_top100_clean_words.xlsx"
    TEST_XLSX = f"{DATA_ROOT}\\test_data.xlsx"
    batch_size = 32
    num_classes = 100
    epochs = 250
    ...
    full_dataset = SignDataset(root_dir=TRAIN_DIR, label_file=TRAIN_XLSX)
    ...
    test_dataset = SignDataset(root_dir=TEST_DIR, label_file=TEST_XLSX)
    ...
    # 改后（增加特征选择步骤，获得 topk_indices 和新的 input_dim）
    # ====== 新增 LightGBM 特征选择流程 ======
    ...
    X_train_k, topk_indices = select_topk_lightgbm(all_train_X, all_train_y,
                                                   k=topk)  # LightGBM으로 top-k 특성 선택 // 用LightGBM选出top-k特征
    torch.save(topk_indices, "topk_indices.pth")
    print("topk_indices.pth 저장")
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,
                              num_workers=8, drop_last=True, collate_fn=collate_fn_topk)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False,
                            num_workers=8, drop_last=False, collate_fn=collate_fn_topk)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False,
                             num_workers=8, drop_last=False, collate_fn=collate_fn_topk)

    model = SignTransformer(input_dim=input_dim, d_model=256, num_classes=num_classes).to(device)
